  ...
  return function(files, metalsmith, done) {
    ...
    _.forEach(tbConvertedFiles, function(file) {
      ...
      if (options.isGroupMaster(fileInCwd)) {
        if (_.isObject(groupMasters[fileGroup])) {
        ...
        data.$groupUrl = options.getGroupUrl(data);
    ...
    var groupsWithoutMaster = _.filter(Object.keys(groupMembers), function(aGroup) {
      return !_.isObject(groupMasters[aGroup]);
    });
...
function normalize(options) {
  var defaults = {
    ...
    getGroupUrl: function(masterFile) {
      var algorithm = "md5";
      var encoding = "base64";
      var hash = require("crypto").createHash(algorithm);
      hash.update(masterFile.contents);
      var base64 = hash.digest(encoding);
      var base64safe = base64.replace(/\+/gi, "-").replace(/\//gi, "_").substr(0, base64.length - 2);
      return masterFile.$groupId[0] + "/" + masterFile.$groupId + "-" + base64safe;
    }
  };
